"""Vectorized outcome engine for Step 2b — Caution Exclusion Filter.

Enters a short at each eligible bar's close and scans forward until
stop or target is resolved.

Stop   = entry + stop_mult × ATR(entry_tf, atr_period)
Target = entry − target_mult × ATR(entry_tf, atr_period)

Populations per pair:
    regime_only  — eligible regime baseline (post warmup)
    ema20_filter — regime + close ≤ EMA(20)
    range_filter — regime + range7 ≤ ATR×1.5
    no_caution   — regime + both conditions

Each result record carries both ``n_trades`` (resolved outcomes) and
``mask_count`` (raw True-bars in the population mask) so that coverage
can be computed accurately without the unresolved-trade bias.
"""
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the caution exclusion check across all configured pairs.

    Returns:
        DataFrame indexed by (pair, population) with columns:
        win_rate, profit_factor, avg_duration, n_trades, mask_count.
    """
    params = Parameters(
    ...
    if not records:
        return pd.DataFrame(
            columns=[
                "pair", "population", "win_rate", "profit_factor",
                "avg_duration", "n_trades", "mask_count",
            ]
        )
...
def _process_pair(
    ...
    for pop_name in _POPULATION_ORDER:
        ...
        # mask_count uses raw mask sum — unaffected by unresolved-trade exclusion.
        # Use this for coverage calculations, not n_trades.
        result["mask_count"] = int(mask.sum())
        records.append({"pair": pair, "population": pop_name, **result})
        logger.debug(
            "%s | %-14s | bars=%d trades=%d win=%.1f%% PF=%.2f dur=%.1f",
            pair,
            pop_name,
            result["mask_count"],
            result["n_trades"],
...
# ---------------------------------------------------------------------------
# Outcome engine (identical pattern to Steps 1, 2a, and 3)
# ---------------------------------------------------------------------------
...
def _compute_outcomes(
    ...
    Enters short at close.  Scans forward until high ≥ stop (loss) or
    low ≤ target (win).  Ties on the same bar → loss (conservative).
    Unresolved trades (end of sample) are excluded.
    """
...
# ---------------------------------------------------------------------------
# Data loading helpers (identical to other steps)
# ---------------------------------------------------------------------------
