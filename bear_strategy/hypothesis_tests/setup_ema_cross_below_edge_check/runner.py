"""Vectorized outcome engine for the EMA Cross-Below Setup Edge Check.
...
Populations per pair:
    regime_only               — eligible regime baseline
    ema_cross_below           — regime + any downward EMA(ema_period) cross
    ema_cross_below_sustained — regime + cross after ≥ min_bars_above bars above EMA
"""
...
from bear_strategy.hypothesis_tests.setup_ema_cross_below_edge_check.entries import (
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the EMA cross-below edge check across all configured pairs."""
    params = Parameters(
...
def _process_pair(pair: str, config: TestConfig, params: Parameters) -> list[dict]:
    ...
    for pop_name in _POPULATION_ORDER:
        ...
        logger.debug(
            "%s | %-26s | trades=%d  win=%.1f%%  PF=%.2f  dur=%.1f bars",
            pair, pop_name, result["n_trades"],
            result["win_rate"] * 100 if not np.isnan(result["win_rate"]) else float("nan"),
            result["profit_factor"], result["avg_duration"],
        )

    return records


def _compute_outcomes(df: pd.DataFrame, entry_mask: pd.Series, config: TestConfig) -> dict:
...
def _load_parquet(data_dir: Path, symbol: str, timeframe: str,
