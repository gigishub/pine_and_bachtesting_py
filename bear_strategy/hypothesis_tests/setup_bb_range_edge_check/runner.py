"""Vectorized outcome engine for the BB Range Setup Edge Check.
...
Populations per pair:
    ...
    price_in_bands — regime + BB_lower ≤ close ≤ BB_upper
    below_upper    — regime + close ≤ BB_upper (no lower guard)
    above_lower    — regime + close ≥ BB_lower (no upper cap)

Look-ahead prevention:
    BB and ATR are backward-looking computations on entry_tf closes.
    No shifting is required — bar t's signal uses only bar t and prior data.
"""
...
from bear_strategy.hypothesis_tests.setup_bb_range_edge_check.entries import (
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the BB range setup edge check across all configured pairs.
...
def _process_pair(
    ...
    for pop_name in _POPULATION_ORDER:
        ...
        logger.debug(
            "%s | %-16s | trades=%d  win=%.1f%%  PF=%.2f  dur=%.1f bars",
            pair,
            pop_name,
            result["n_trades"],
