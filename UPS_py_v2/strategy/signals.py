"""Signal orchestrator for the UPS strategy.

Calls each indicator sub-module and merges results into a single dict of
pandas Series. This is the only module that imports from all other strategy
sub-packages — callers should import build_strategy_series from here.
"""
...
from .indicators.ma import compute_base_and_ma_context
...
def build_strategy_series(
    ...
    hammer_fib: float,
    hammer_size: float,
    # Stops & targets (passed through for future use; not precomputed)
    ...
    """Build all precomputed signal series used by the backtest and live runner.

    Returns a dict keyed by series name. All series share the same DatetimeIndex
    as `df`.
    """
