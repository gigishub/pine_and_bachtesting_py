"""Vectorized outcome engine for the RVOL Setup Edge Check.
...
Populations per pair:
    regime_only        — eligible regime baseline (post vol_ma/ATR warmup)
    rvol_spike         — regime + RVOL ≥ threshold (for min_bars_active bars)
    rvol_spike_bearish — regime + rvol_spike + bearish candle (close < open)
    rvol_spike_down    — regime + rvol_spike + price declined (close < prev close)

Look-ahead prevention:
    vol_ma at bar t uses Volume[t-N+1 … t] — backward window only.
    rvol[t] = Volume[t] / vol_ma[t] — both known at bar t's close.
    rolling min for min_bars_active uses the same backward window.
"""
...
from bear_strategy.hypothesis_tests.setup_rvol_edge_check.entries import build_population_masks
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the RVOL setup edge check across all configured pairs.
