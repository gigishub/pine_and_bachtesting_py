"""Vectorized outcome engine for Setup 2 Candle Trigger.

Entry TF  : 15m
KDE TF    : 4h  (shifted 1 completed 4h bar, forward-filled to 15m)
Baseline  : regime AND kde_upper — all populations are subsets.
"""
...
from bear_strategy.hypothesis_tests.setup_2_candle_trigger_check.entries import build_population_masks
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the candle trigger check across all configured pairs."""
    params = Parameters(
...
def _process_pair(
    ...
    # KDE signals on 4h: compute, shift 1 bar, align to 15m
    ...
    kde_upper_4h = (
        pd.Series(kde_result["setup_active_upper"], index=df_4h.index, name="kde_upper")
        .shift(1)
        .to_frame()
    )
...
def _align_to_entry_tf(df_entry: pd.DataFrame, series_or_df) -> pd.DataFrame:
