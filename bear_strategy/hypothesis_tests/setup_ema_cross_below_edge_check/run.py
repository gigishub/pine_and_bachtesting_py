...
Usage:
    ...
    python -m bear_strategy.hypothesis_tests.setup_ema_cross_below_edge_check.run

Timeframe (set entry_tf in config.py):
    "1h"  — swing shorts  ← default
    "15m" — intraday scalps (increase min_bars_above to 8+)
    "4h"  — position shorts (min_bars_above = 3–5 is fine)
...
a bearish EMA cross (close drops below EMA after being above it) creates
edge — and whether requiring N consecutive bars above before the cross
(a genuine counter-trend rally) makes the signal stronger.

    ema_cross_below           — any cross below EMA(ema_period).
    ema_cross_below_sustained — cross after ≥ min_bars_above bars above EMA.

The hypothesis: in a bear regime, when a counter-trend bounce (sustained
time above EMA) fails and price re-crosses below the EMA, that failure
is a high-conviction continuation signal.
...
def _print_summary(results: pd.DataFrame, config: TestConfig) -> None:
    ...
    ordered_pops = ["regime_only", "ema_cross_below", "ema_cross_below_sustained"]
    ...
    for pair in config.pairs:
        ...
        if regime_n > 0:
            for pop in ["ema_cross_below", "ema_cross_below_sustained"]:
...
def _print_verdict(results: pd.DataFrame, config: TestConfig) -> None:
    ...
    print("── EMA Cross-Below Setup Edge Verdict ──\n")
    print(
        f"  Baseline (regime_only):  avg WR {baseline['wr_base'].mean() * 100:.1f}%  "
        f"avg PF {baseline['pf_base'].mean():.3f}"
    )
    print(f"  Config: EMA({config.ema_period})  min_bars_above={config.min_bars_above}")
    print()

    populations_to_test = ["ema_cross_below", "ema_cross_below_sustained"]
    ...
    for pop in populations_to_test:
        ...
        print(
            f"  {pop:<28}  avg WR lift {avg_wr_lift:+.2f}pp  avg PF {avg_pf:.3f}"
...
def _print_final_verdict(passing: list[tuple[str, float]], config: TestConfig, warnings: list[str]) -> None:
    if not passing:
        print("  ❌  EMA CROSS-BELOW EDGE NOT CONFIRMED — no population clears all thresholds.")
        print("      Next steps:")
        print(f"        • Try min_bars_above = {config.min_bars_above + 2} (require longer rally before cross)")
        print(f"        • Try ema_period = 10 or 50")
        print("        • Try a different entry_tf in config.py")
        print("        • EMA crosses may need a trigger (volume / momentum) to add real edge")
        print()
        return

    print("  ✅  EMA CROSS-BELOW EDGE CONFIRMED — at least one population creates predictive edge.")
    ...
    if "ema_cross_below" in pop_names:
        print("      ema_cross_below           ✅ — any EMA cross adds edge in bear regime.")
        print("                                     Even 1-bar bounces that fail are informative.")
    if "ema_cross_below_sustained" in pop_names:
        print(
            f"      ema_cross_below_sustained ✅ — cross after ≥{config.min_bars_above} bars above "
            "is the stronger signal."
        )
        print("                                     Failed counter-trend rallies predict continuation.")
        if "ema_cross_below" not in pop_names:
            print("                                     Note: raw cross failed — the N-bar filter is essential.")

    not_passing = [p for p in ["ema_cross_below", "ema_cross_below_sustained"] if p not in pop_names]
    for p in not_passing:
        label = {
            "ema_cross_below":           "ema_cross_below           ❌ — raw EMA cross does not add edge alone.",
            "ema_cross_below_sustained": "ema_cross_below_sustained ❌ — sustained cross does not improve edge.",
        }[p]
        print(f"      {label}")
    print()
