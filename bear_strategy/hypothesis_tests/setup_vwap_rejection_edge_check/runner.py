"""Vectorized outcome engine for the VWAP Rejection Candle Setup Edge Check.
...
Populations per pair:
    ...
    vwap_rejection_clean — regime + rejection + both wicks ≤ max_wick_ratio
"""
...
from bear_strategy.hypothesis_tests.setup_vwap_rejection_edge_check.entries import (
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the VWAP rejection candle edge check across all configured pairs.
...
def _process_pair(
    ...
    for pop_name in _POPULATION_ORDER:
        ...
        logger.debug(
            "%s | %-24s | trades=%d  win=%.1f%%  PF=%.2f  dur=%.1f bars",
            pair,
            pop_name,
            result["n_trades"],
