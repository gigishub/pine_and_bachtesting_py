"""Vectorized outcome engine for the RSI Setup Edge Check.
...
Populations per pair:
    regime_only  — eligible regime baseline (post RSI/ATR warmup)
    rsi_30_50    — regime + RSI strictly between rsi_lower and rsi_upper
    rsi_below_50 — regime + RSI < rsi_upper (no lower guard)
    rsi_above_30 — regime + RSI > rsi_lower (no upper cap)

Look-ahead prevention:
    RSI and ATR are backward-looking EWM computations on entry_tf closes.
...
from bear_strategy.hypothesis_tests.setup_rsi_edge_check.entries import (
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the RSI setup edge check across all configured pairs.
