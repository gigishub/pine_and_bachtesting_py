"""CLI entry point for Setup 2 – Trigger 3: oscillator-level and BB-expansion signals.

Usage:
    source .venv/bin/activate
    python -m bear_strategy.hypothesis_tests.setup_2_trigger_3_check.run
...
Key parameters (all in config.py):
    ...
    rsi_threshold   -- RSI cross-below level (default: 50)
    mfi_period      -- MFI lookback (default: 14)
    mfi_threshold   -- MFI cross-below level (default: 50)
...
Four signals tested on top:
  ...
  rsi_cross_ma    RSI crosses below its own MA(rsi_ma_period)
  bb_expand_down  BB lower band expands down + close < midband + bearish bar
...
from bear_strategy.hypothesis_tests.setup_2_trigger_3_check.runner import run_test
...
_GROUP = {
    "rsi_cross_50":   "Oscillator  ",
    "mfi_cross_50":   "Volume      ",
    "rsi_cross_ma":   "Oscillator  ",
    "bb_expand_down": "Volatility  ",
    "ema_rvol_cross": "Trend+Vol   ",
}
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("Bear Strategy -- Setup 2 Trigger 3: oscillator + BB signals")
    ...
    logger.info(
        "rsi_cross_50   : RSI(%d) crosses below %.0f",
        config.rsi_period, config.rsi_threshold,
    )
    logger.info(
        "mfi_cross_50   : MFI(%d) crosses below %.0f",
        config.mfi_period, config.mfi_threshold,
    )
    logger.info(
        "rsi_cross_ma   : RSI(%d) crosses below %s(%d) of RSI",
        config.rsi_period, config.rsi_ma_type.upper(), config.rsi_ma_period,
    )
    logger.info(
        "bb_expand_down : BB(%d, %s std) lower band expands down + bearish close < midband",
        config.bb_period, bb_str,
    )
    logger.info(
        "ema_rvol_cross : price crosses below EMA(%d) with RVOL(%d) > %.1f",
        config.ema_rvol_period, config.ema_rvol_lookback, config.ema_rvol_threshold,
    )
    logger.info(
    ...
    stem = run_stem(
        ...
        extra=(
            f"trigger3_kde{config.kde_window}_bw{bw_str}"
            f"_rsi{config.rsi_period}x{_fmt_param(config.rsi_threshold)}"
            f"_mfi{config.mfi_period}x{_fmt_param(config.mfi_threshold)}"
            f"_rsiMA{config.rsi_ma_type}{config.rsi_ma_period}"
            f"_bb{config.bb_period}s{bb_str}"
            f"_emaRvol{config.ema_rvol_period}lb{config.ema_rvol_lookback}x{_fmt_param(config.ema_rvol_threshold)}"
        ),
    )
    _config_params = {
        ...
        "ema_rvol_period":    config.ema_rvol_period,
        "ema_rvol_lookback":  config.ema_rvol_lookback,
        "ema_rvol_threshold": config.ema_rvol_threshold,
    }
    ...
    md_path = config.results_dir / f"trigger3_results_{stem}.md"
    save_report(
        buf.text,
        md_path,
        (
            f"Bear Strategy -- Setup 2 Trigger 3  "
        ...
        if baseline_n > 0:
            ...
            for pop in _TEST_POPULATIONS:
                if pop in pair_df.index:
                    ...
                    print(f"    {pop:<16}  covers {cov * 100:.1f}% of baseline bars{flag}")
        print()
...
def _print_verdict(results: pd.DataFrame, config: TestConfig) -> None:
    ...
    print(f"\n-- Setup 2 Trigger 3 ({config.entry_tf}) -- Verdict --\n")
    print(
    ...
    print(
        ...
        f"MFI({config.mfi_period})<{config.mfi_threshold:.0f}  "
        f"RSI/{config.rsi_ma_type.upper()}({config.rsi_ma_period})  "
        f"BB({config.bb_period},{bb_str}std)"
    )
    print()
    ...
    for pop in _TEST_POPULATIONS:
        ...
        if pop_df.empty:
            print(f"  [{_GROUP[pop]}]  {pop:<16}  -- no data")
            continue
...
def _save_results(results: pd.DataFrame, config: TestConfig, stem: str) -> None:
    ...
    out = csv_dir / f"trigger3_results_{stem}.csv"
...
def _print_final_verdict(
    ...
    if not passing:
        print(
            f"  [XX]  SETUP 2 TRIGGER 3 ({config.entry_tf}) -- "
            "no oscillator/band signal improves on kde_upper_baseline."
        )
        print(f"        Baseline avg PF was {base_pf:.3f}.")
        print("      Tuning ideas:")
        print(f"        * Adjust RSI threshold (currently {config.rsi_threshold:.0f})")
        print(f"        * Adjust MFI threshold (currently {config.mfi_threshold:.0f})")
        print(f"        * Change rsi_ma_type to 'sma' (currently {config.rsi_ma_type})")
        print(f"        * Widen BB std (currently {config.bb_std})")
        print(f"        * Adjust EMA period (currently {config.ema_rvol_period})")
        print(f"        * Lower RVOL threshold (currently {config.ema_rvol_threshold})")
    ...
    print(
        f"  [OK]  SETUP 2 TRIGGER 3 ({config.entry_tf}) CONFIRMED -- "
    ...
    groups = {
        "Oscillator": {"rsi_cross_50", "rsi_cross_ma"},
        "Volume":     {"mfi_cross_50"},
        "Volatility": {"bb_expand_down"},
        "Trend+Vol":  {"ema_rvol_cross"},
    }
    for group_label, members in groups.items():
        ...
        if confirmed or failed_m:
            ...
            for p in sorted(confirmed):
                ...
                print(f"      [OK] {p:<18}  avg PF {pf_val:.3f}")
            for p in sorted(failed_m):
                print(f"      [--] {p}")

    print()
    print("  Proceed using confirmed trigger(s) for setup 3 construction.")
    print()
