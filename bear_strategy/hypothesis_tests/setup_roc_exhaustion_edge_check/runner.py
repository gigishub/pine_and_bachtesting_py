"""Vectorized outcome engine for the ROC Exhaustion Setup Edge Check.
...
Populations per pair:
    regime_only    — eligible regime baseline (post ROC/ATR warmup)
    roc_post_bull  — regime + bullExhaustionActive (post-bull ROC red zone)
    roc_bear_trend — regime + isBearTrend (active ROC bear momentum)
    roc_post_bear  — regime + bearExhaustionActive (post-bear ROC green zone)

Look-ahead prevention:
    ROC at bar t = (close[t] / close[t - roc_len] - 1) × 100.  Both prices
    are known at bar t's close.  The exhaustion state machine only reads the
    previous bar's state and current/prior trend flags — no future data used.
"""
...
from bear_strategy.hypothesis_tests.setup_roc_exhaustion_edge_check.entries import (
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the ROC exhaustion setup edge check across all configured pairs.
...
def _compute_outcomes(
    ...
    Enters a short at each bar's close.  Scans forward until high ≥ stop
    (loss) or low ≤ target (win).  Ties on the same bar = loss (conservative).
