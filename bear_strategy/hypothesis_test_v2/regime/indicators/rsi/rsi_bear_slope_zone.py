"""
rsi_bear_slope_zone — Regime indicator.

What it measures
----------------
RSI in bearish zone (30–50) AND RSI MA in zone AND RSI MA slope is negative
(declining smoothed RSI confirms sustained bearish pressure, not just a dip).

The slope filter removes bars where RSI MA is in the zone but drifting upward —
those may be early recovery attempts, not confirmed bear regimes.
...
True  = RSI ∈ zone AND RSI_MA ∈ zone AND RSI_MA is declining.
...
def signal(df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    slope_down     = rsi_ma.diff() < 0   # RSI MA is declining

    return (in_zone_rsi & in_zone_rsi_ma & slope_down).fillna(False)
