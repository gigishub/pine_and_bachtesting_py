...
# Bear Strategy Live Run Controller
# Runtime state lives under: bear_strategy/live/control/registry

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../../.." && pwd)"
VENV_ACTIVATE="${VIRTUAL_ENV:-$PROJECT_DIR/.venv}/bin/activate"
...
Usage:
  ...
  runctl.sh profile-start <profile_name> [--run-name NAME] [--auto-restart yes|no]
...
Examples:
  ./runctl.sh profile-start NEAR_1h
  ./runctl.sh profile-start ATOM_1h
  ./runctl.sh status
  ./runctl.sh stop NEAR_1h
  ./runctl.sh resurrect
...
profile_command() {
  ...
  printf '%s' "cd \"$PROJECT_DIR\" && source \"$VENV_ACTIVATE\" && set -a && source \"$profile_file\" && set +a && python -m bear_strategy.live.bear_live_runner"
}
...
cmd_profile_list() {
  find "$PROFILES_DIR" -maxdepth 1 -type f -name '*.env' -exec basename {} \; | sed 's/\.env$//' | sort
}
...
last_finished_attempt() {
  ...
  local lf
  lf="$(find "$INACTIVE_DIR/$run_name" -mindepth 1 -maxdepth 1 -type d -exec basename {} \; | sort | tail -n 1 || true)"
  [[ -n "$lf" ]] && echo "$lf" || echo "-"
}

status_table() {
  {
    echo "Bear Strategy Live Runner — Runtime Status"
    ...
    while IFS= read -r n; do
      [[ -n "$n" ]] && names+=("$n")
    done < <(find "$CONFIGS_DIR" -maxdepth 1 -type f -name '*.conf' -exec basename {} \; | sed 's/\.conf$//' | sort)
...
cmd_start_all() {
  while IFS= read -r profile; do
    ...
    cmd_profile_start "$profile"
  done < <(cmd_profile_list)

  status_table > /dev/null
}

cmd_install_systemd() {
  local service_name="bear-live-control.service"
  local service_path="/etc/systemd/system/${service_name}"

  cat > "/tmp/${service_name}" <<EOF
[Unit]
Description=Bear Strategy Live Run Controller (resurrect wanted runs)
