"""Entry point for downloading Bybit open interest history.

Automatically detects which symbols already have OHLCV data and downloads
the corresponding open interest for those same pairs.

Run from the project root with the virtual environment active:

    cd /Users/andre/Documents/Python_local/pine_script
    source .venv/bin/activate && python -m crypto_data.oi_main

Files are saved to:
    crypto_data/data/<SYMBOL>/<SYMBOL>_oi_<interval>_start_<date>_end_<date>.parquet
"""
...
from crypto_data.oi_downloader import download_open_interest
...
if __name__ == "__main__":
    ...
    logger.info("Symbols to fetch OI for: %s", symbols)
