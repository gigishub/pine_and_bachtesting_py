"""Vectorized outcome engine for the EMA20 / VWAP Setup Edge Check.
...
Populations per pair:
    regime_only  — eligible regime baseline (post EMA20/ATR warmup)
    below_ema20  — regime + close < EMA(20)
    below_vwap   — regime + close < daily-anchored VWAP
    below_both   — regime + close < EMA(20) AND close < VWAP
"""
...
from bear_strategy.hypothesis_tests.setup_ema_vwap_edge_check.entries import build_population_masks
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the EMA20/VWAP setup edge check across all configured pairs.
