"""Vectorized outcome engine for the VWAP Cross-Below Setup Edge Check."""
...
from bear_strategy.hypothesis_tests.setup_vwap_cross_edge_check.entries import (
...
def _process_pair(pair: str, config: TestConfig, params: Parameters) -> list[dict]:
    ...
    for pop_name in _POPULATION_ORDER:
        result = _compute_outcomes(df, masks[pop_name], config)
...
def _compute_outcomes(df: pd.DataFrame, entry_mask: pd.Series, config: TestConfig) -> dict:
    ...
    entry_indices = np.where(entry_mask.to_numpy(dtype=bool))[0]
...
def _standardise_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    rename_map = {"open": "Open", "high": "High", "low": "Low", "close": "Close",
                  "volume": "Volume", "Open time": "timestamp", "open_time": "timestamp"}
