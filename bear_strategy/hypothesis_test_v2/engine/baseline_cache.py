...
----------
    ...
    b_wins, b_losses, b_pf, b_wr = get_or_compute_baseline(...)
"""
...
def get_or_compute_baseline(
    ...
    baseline_mask:  Boolean Series marking which bars form the baseline population.
                    For regime phase: pd.Series(True, index=df.index) — all bars.
                    For setup phase:  the regime-filtered bars.
                    For trigger phase: regime & setup filtered bars.
    baseline_label: Human-readable string used in the cache filename, e.g.
                    "all", "close_below_ema_50", "ema50_rsi_range".
