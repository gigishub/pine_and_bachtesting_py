"""
Unit tests for hypothesis_test_v2 HTF alignment engine.
...
from bear_strategy.hypothesis_test_v2.engine.alignment import (
    align_htf_series,
...
class TestValidateNoLookahead:
    def test_passes_for_shifted_signal(self) -> None:
        ...
        # Signal that is always False — trivially no lookahead
    ...
    def test_raises_for_known_future_leak(self) -> None:
        """
        If a signal fires on the exact open of a HTF bar without shift,
        validate_no_lookahead should detect it and raise LookaheadError.
        """
        ltf_df = pd.DataFrame(
            {"close": 100.0},
            index=pd.date_range("2021-01-01", periods=48, freq="1h"),
