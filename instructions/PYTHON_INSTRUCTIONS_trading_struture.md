...
In a trading robot this might be:
- `UPS_py_v2/` — project root
- `UPS_py_v2/live/ups_runner/` — module root for live trading logic
- `UPS_py_v2/backtest/` — module root for backtesting logic

**Do not create a new top-level directory unless the feature is clearly a new domain.** `backtest/`, `live/`, `strategy/` are domains. A new exchange connector or signal filter is not — it belongs inside an existing one.
...
One-off utilities (manual backfills, exchange diagnostics, data migrations) that don't belong in any module go in `scripts/` at the project root — not in `main.py` and not cluttering a module.
...
In a trading context: **fetch**, **parse**, **signal**, **execute**, and **log** should be separate concerns — never collapsed into one function or file.
...
**Never silence exceptions.** A bare `except: pass` in trading code means a failed order or corrupted state goes completely unnoticed.
...
- Never suppress an exception in order execution, position tracking, or state management
...
│   ├── fetch.py
│   └── tests/
│       └── test_fetch.py           ← scoped to backtest
├── live/ups_runner/
│   └── tests/
│       └── test_order_manager.py   ← scoped to live trading
```
...
- **Delete dead code, don't comment it out** — git has the history. Commented-out logic in live trading files is a reliability hazard
...
Keep a `CHANGELOG.md` at the project root. In a live trading system, knowing *when* behavior changed and *why* is essential for diagnosing discrepancies between backtest and live results.
...
Minimum entry: **date — what changed — why it changed.** This is especially important for strategy parameter changes and exchange API updates.
