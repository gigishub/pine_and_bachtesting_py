"""Vectorized outcome engine for the BB Widening Setup Edge Check.
...
Populations per pair:
    regime_only          — eligible regime baseline (post BB/ATR warmup)
    bb_widening          — regime + BB bands expanding (width > width[1])
    bb_widening_bearish  — regime + widening + close < BB_basis
    bb_widening_breakout — regime + widening + close < BB_lower

Look-ahead prevention:
    All signals are computed on entry_tf bars at bar close.  No higher-TF
    alignment is needed.  BB and ATR are backward-looking from bar t.
    ATR uses EWM(span=atr_period) for consistency with all other steps.
"""
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the BB setup edge check across all configured pairs.
...
def _process_pair(
    ...
    # Regime signals mapped from daily onto entry_tf (same as Steps 1 and 3)
    regime = compute_regime_signals(df, df_daily, params)
    df = df.copy()
    for col, series in regime.items():
        df[col] = series.values

    # ATR on entry_tf bars — used for stop/target sizing
...
# ---------------------------------------------------------------------------
# Outcome engine (identical to Steps 1 and 3)
# ---------------------------------------------------------------------------
