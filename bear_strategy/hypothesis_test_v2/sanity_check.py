"""
Lookahead / bias sanity check for hypothesis_test_v2.
...
2. Shifted entry (pure future signal)
   Enters N bars *before* a signal fires — the entry bar cannot possibly know about
   the future signal, so this should perform at or below random.
...
from bear_strategy.hypothesis_test_v2.engine.outcome_engine import compute_atr, compute_outcomes
...
def test_shifted_entry(df: pd.DataFrame) -> bool:
    ...
    # Use close < 200-bar EMA as a simple regime signal
    ...
    # Shifted entries look N bars into the future — they should not be dramatically better
    # than the real signal (which is also filtered by close<ema200).
    # A pathological lookahead would show shifted >> real.
    ok = pf_shifted <= pf_real * 1.10  # allow 10% tolerance for statistical noise
    return _passes(
        f"Shifted-entry PF vs real-entry PF",
        ok,
    ...
    try:
        ...
        for orig, bak in backed_up:
            if os.path.exists(orig):
                os.remove(orig)   # remove the freshly written one
            os.rename(bak, orig)  # restore backup
...
def main() -> None:
    print("\n═══════════════════════════════════════════════════")
    print("  hypothesis_test_v2 — Lookahead / Bias Sanity Check")
    ...
    cache_dir = "bear_strategy/hypothesis_test_v2/regime/results/baseline_cache"
