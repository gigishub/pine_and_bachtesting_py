"""
Vectorised long-trade outcome engine.

For each bar where *entry_mask* is True, simulates a long trade:
  - Stop loss:   entry_price - stop_atr_mult  * ATR  (below entry)
  - Take profit: entry_price + target_atr_mult * ATR  (above entry)
...
def _simulate_trades(
    ...
    """
    Forward-scan simulator for long trades.
    ...
    Stop loss:   entry_price - stop_mult   * ATR  (below entry)
    Take profit: entry_price + target_mult * ATR  (above entry)
    ...
    for idx in entry_indices:
        ...
        stop_price   = entry_price - stop_mult   * atr_val  # below entry
        target_price = entry_price + target_mult * atr_val  # above entry

        for j in range(idx + 1, n):
            bar_high = high[j]
            bar_low  = low[j]

            hit_stop   = bar_low  <= stop_price    # price drops to stop
            hit_target = bar_high >= target_price  # price rises to target
...
def compute_outcomes(
    ...
    """
    Compute candidate vs baseline outcome statistics for long trades.
    ...
    stop_mult:      ATR multiples for stop loss (below entry).
    target_mult:    ATR multiples for take-profit target (above entry).
