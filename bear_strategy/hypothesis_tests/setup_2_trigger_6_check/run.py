"""CLI entry point for Setup 2 – Trigger 6: MACD momentum signals.

Usage:
    source .venv/bin/activate
    python -m bear_strategy.hypothesis_tests.setup_2_trigger_6_check.run

Change entry_tf in config.py to test 15m / 1h / 4h.

Key parameters (all in config.py):
    entry_tf                       -- entry timeframe (default: 1h)
    macd_fast                      -- fast EMA for MACD (default: 12)
    macd_slow                      -- slow EMA for MACD (default: 26)
    macd_signal                    -- signal line EMA (default: 9)
    histogram_lookback             -- lookback for peak_roll/twin_peaks (default: 20)
    cross_velocity_slope_threshold -- min slope for steep cross (default: 0.05)
    zero_line_margin               -- margin for zero-line rejection (default: 0.01)
...
Six MACD-based signals tested on top (all condition-based):

  zero_line_rejection        MACD touches 0 from below, turns lower (kiss of death)
  histogram_peak_roll        Histogram[1] > Histogram while positive (exhaustion)
  rapid_separation           MACD gap widening (acceleration)
  signal_line_wall           Signal < 0 AND MACD < Signal (regime filter)
  histogram_twin_peaks       Price HH but Histogram LH (bearish divergence)
  cross_velocity             MACD crosses below Signal with steep slope (violent)
...
from bear_strategy.hypothesis_tests.setup_2_trigger_6_check.runner import run_test
...
_GROUP = {
    "zero_line_rejection": "Reversal  ",
    "histogram_peak_roll": "Exhaustion",
    "rapid_separation": "Momentum  ",
    "signal_line_wall": "Regime    ",
    "histogram_twin_peaks": "Divergence",
    "cross_velocity": "Reversal  ",
}
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("Bear Strategy -- Setup 2 Trigger 6: MACD momentum signals")
    ...
    logger.info("MACD     : fast=%d  slow=%d  signal=%d", config.macd_fast, config.macd_slow, config.macd_signal)
    logger.info("zero_line_rejection   : MACD touches 0 from below, turns lower")
    logger.info("histogram_peak_roll   : Histogram[1] > Histogram while positive")
    logger.info("rapid_separation      : MACD gap widening (acceleration)")
    logger.info("signal_line_wall      : Signal < 0 AND MACD < Signal")
    logger.info("histogram_twin_peaks  : Price HH but Histogram LH (divergence)")
    logger.info("cross_velocity        : MACD crosses below Signal with steep slope")
    logger.info(
    ...
    stem = run_stem(
        ...
        extra=(
            f"trigger6_kde{config.kde_window}_bw{bw_str}"
            f"_macd{config.macd_fast}_{config.macd_slow}_{config.macd_signal}"
            f"_hlb{config.histogram_lookback}"
        ),
    )
    _config_params = {
        ...
        "histogram_lookback": config.histogram_lookback,
        "cross_velocity_slope_threshold": config.cross_velocity_slope_threshold,
        "zero_line_margin":   config.zero_line_margin,
    }
    ...
    md_path = config.results_dir / f"trigger6_results_{stem}.md"
    save_report(
        buf.text,
        md_path,
        (
            f"Bear Strategy -- Setup 2 Trigger 6  "
...
def _print_verdict(results: pd.DataFrame, config: TestConfig) -> None:
    ...
    print(f"\n-- Setup 2 Trigger 6 ({config.entry_tf}) -- Verdict --\n")
    print(
...
def _save_results(results: pd.DataFrame, config: TestConfig, stem: str) -> None:
    ...
    out = csv_dir / f"trigger6_results_{stem}.csv"
...
def _print_final_verdict(
    ...
    if not passing:
        print(
            f"  [XX]  SETUP 2 TRIGGER 6 ({config.entry_tf}) -- "
            "no MACD signal improves on kde_upper_baseline."
        )
        print(f"        Baseline avg PF was {base_pf:.3f}.")
        print("      Tuning ideas:")
        print(f"        * Adjust MACD periods (currently {config.macd_fast}/{config.macd_slow}/{config.macd_signal})")
        print(f"        * Adjust histogram lookback (currently {config.histogram_lookback})")
        print(f"        * Adjust cross_velocity slope threshold (currently {config.cross_velocity_slope_threshold})")
    ...
    print(
        f"  [OK]  SETUP 2 TRIGGER 6 ({config.entry_tf}) CONFIRMED -- "
    ...
    groups = {
        "Reversal": [p for p in _TEST_POPULATIONS if _GROUP[p] == "Reversal  "],
        "Exhaustion": [p for p in _TEST_POPULATIONS if _GROUP[p] == "Exhaustion"],
        "Momentum": [p for p in _TEST_POPULATIONS if _GROUP[p] == "Momentum  "],
        "Regime": [p for p in _TEST_POPULATIONS if _GROUP[p] == "Regime    "],
        "Divergence": [p for p in _TEST_POPULATIONS if _GROUP[p] == "Divergence"],
    }
    for group_label, members in groups.items():
        confirmed = {m for m in members if m in passing_pops}
        failed_m  = {m for m in members if m not in passing_pops}
    ...
    print()
    print("  Proceed using confirmed MACD signal(s) for setup 3 construction.")
    print()
