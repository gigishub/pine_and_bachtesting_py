"""CLI entry point for Setup 2 – Trigger 5: BB + Keltner Channel signals.

Usage:
    source .venv/bin/activate
    python -m bear_strategy.hypothesis_tests.setup_2_trigger_5_check.run

Change entry_tf in config.py to test 15m / 1h / 4h.

Key parameters (all in config.py):
    entry_tf               -- entry timeframe (default: 1h)
    bb_period              -- Bollinger Band SMA/std window (default: 20)
    bb_std                 -- Band width in std deviations (default: 2.0)
    kc_period              -- Keltner Channel ATR window (default: 20)
    kc_atr_mult            -- ATR multiplier for channel (default: 2.0)
    vol_vel_lookback       -- rolling avg for BB width delta (default: 20)
    squeeze_lookback       -- lookback for BB width minimum (default: 20)
...
Six volatility/expansion signals tested on top (all held-state):

  momentum_close           Close < Lower_Band (price acceptance at lows)
  falling_tunnel           Both BB bands moving down together
  volatility_velocity      BB width expansion rate accelerating
  squeeze_snap             BB contraction then expansion snap
  lower_expansion          Lower band dropping (broadest trigger)
  keltner_squeeze_release  Lower BB drops below Keltner Channel (statistical extreme)
...
from bear_strategy.hypothesis_tests.setup_2_trigger_5_check.runner import run_test
...
_GROUP = {
    "momentum_close":         "Volatility  ",
    "falling_tunnel":         "Volatility  ",
    "volatility_velocity":    "Volatility  ",
    "squeeze_snap":           "Volatility  ",
    "lower_expansion":        "Volatility  ",
    "keltner_squeeze_release": "Volatility  ",
}
...
def main() -> None:
    ...
    kc_str  = _fmt_param(config.kc_atr_mult)

    logger.info("=" * 70)
    logger.info("Bear Strategy -- Setup 2 Trigger 5: BB + Keltner volatility signals")
    ...
    logger.info("BB       : period=%d  std=%.1f  |  KC: period=%d  atr_mult=%.1f",
                config.bb_period, config.bb_std, config.kc_period, config.kc_atr_mult)
    logger.info("momentum_close        : Close < Lower_Band")
    logger.info("falling_tunnel        : Both BB bands moving down")
    logger.info("volatility_velocity   : BB width delta > rolling average")
    logger.info("squeeze_snap          : BB at %d-bar low, then lower band drops", config.squeeze_lookback)
    logger.info("lower_expansion       : Lower_Band < Lower_Band[1]")
    logger.info("keltner_squeeze_release: Lower_BB < Lower_Keltner_Channel")
    logger.info(
    ...
    stem = run_stem(
        ...
        extra=(
            f"trigger5_kde{config.kde_window}_bw{bw_str}"
            f"_bb{config.bb_period}s{bb_str}"
            f"_kc{config.kc_period}a{kc_str}"
            f"_vvlb{config.vol_vel_lookback}_sqlb{config.squeeze_lookback}"
        ),
    )
    _config_params = {
        ...
        "vol_vel_lookback":   config.vol_vel_lookback,
    ...
    md_path = config.results_dir / f"trigger5_results_{stem}.md"
    save_report(
        buf.text,
        md_path,
        (
            f"Bear Strategy -- Setup 2 Trigger 5  "
...
def _print_verdict(results: pd.DataFrame, config: TestConfig) -> None:
    ...
    print(f"\n-- Setup 2 Trigger 5 ({config.entry_tf}) -- Verdict --\n")
    print(
    ...
    print(
        ...
        f"BB({config.bb_period},{bb_str}std)  KC({config.kc_period},{kc_str}atr)"
    )
    print()
...
def _save_results(results: pd.DataFrame, config: TestConfig, stem: str) -> None:
    ...
    out = csv_dir / f"trigger5_results_{stem}.csv"
...
def _print_final_verdict(
    ...
    if not passing:
        print(
            f"  [XX]  SETUP 2 TRIGGER 5 ({config.entry_tf}) -- "
            "no BB/KC signal improves on kde_upper_baseline."
        )
        print(f"        Baseline avg PF was {base_pf:.3f}.")
        print("      Tuning ideas:")
        print(f"        * Adjust BB period (currently {config.bb_period})")
        print(f"        * Adjust BB std (currently {config.bb_std})")
        print(f"        * Adjust KC period (currently {config.kc_period})")
        print(f"        * Adjust KC atr_mult (currently {config.kc_atr_mult})")
    ...
    print(
        f"  [OK]  SETUP 2 TRIGGER 5 ({config.entry_tf}) CONFIRMED -- "
    ...
    print()
    print("  Proceed using confirmed BB/KC signal(s) for setup 3 construction.")
    print()
