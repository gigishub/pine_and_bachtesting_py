"""Vectorized outcome engine for Setup 2 – Trigger 1.

Entry TF : configurable (default 1h)
KDE TF   : 4h  — peak price level shifted 1 completed bar, forward-filled to entry TF
Baseline : regime AND kde_upper — all populations are strict subsets.

KDE output columns aligned to entry TF:
    kde_upper    -- boolean gate (open_4h > kde_peak_4h, from prior 4h bar)
    kde_peak     -- POC price level (from prior 4h bar)
"""
...
from bear_strategy.hypothesis_tests.setup_2_trigger_1_check.entries import build_population_masks
...
def run_test(config: TestConfig) -> pd.DataFrame:
    """Run the trigger-1 check across all configured pairs."""
    params = Parameters(
...
def _process_pair(
    ...
    df_entry["atr_short"] = _atr(df_entry, config.atr_short_period).values
    ...
    df_entry["kde_peak"]  = aligned["kde_peak"].values
